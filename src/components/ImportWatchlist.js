...
const normalizeMediaType = (value) => {
  ...
  if (['movie', 'film', 'movies', 'jpeg', 'jpg', 'png', 'pdf', 'image', 'document'].includes(normalized)) {
    return 'MOVIE';
  }

  if (['tv', 'series', 'show', 'shows'].includes(normalized)) {
    return 'TV';
  }
  ...
  const handleFileSelect = async (file) => {
    ...
    try {
      ...
      const validData = data.map((item) => ({
        ...
        type: normalizeMediaType(item.type || item[1]),
